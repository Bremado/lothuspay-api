package com.lothuspay.wallet.controller.withdrawals;
...
public class WithdrawalsController {
    ...
    @GetMapping
    public Mono<DtoResponse> withdrawals(@RequestParam(value = "page", defaultValue = "1", required = false) int page,
        ...
        return contextHolder.current().flatMap(context -> walletService.withdrawals(context, (finalPage-1), finalLimit))
    ...
    @GetMapping("/{id}")
    public Mono<DtoResponse> withdrawDetails(@PathVariable("id") String id) {
        return contextHolder.current().flatMap(context -> walletService.withdrawDetails(context, id))
                .switchIfEmpty(Mono.just(DtoResponse.builder()
                        .status(DtoResponseStatus.ERR_AUTH_001)
                        .message(DtoResponseStatus.ERR_AUTH_001.getMessage())
                        .build()));
